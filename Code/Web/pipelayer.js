...
function Dot(x, y, radius, gx, gy) {
    ...
    this.update = function () {
        ...
        if (click.x < this.x + this.radius && click.x > this.x - this.radius &&
            ...
            if (origin) {
                //if player one move is valid create link
...
function Box(x, y, hwidth, gx, gy) {
    ...
    this.update = function () {
        ...
        if ((mouse.x - this.x < MOUSE_DISTANCE && mouse.x - this.x > -MOUSE_DISTANCE &&
            mouse.y - this.y < MOUSE_DISTANCE && mouse.y - this.y > -MOUSE_DISTANCE) ||
            origin === this) {
            this.hwidth = CUBE_HALF * 1.5;
        } else {
            this.hwidth = CUBE_HALF;
        }

        //handle click
        if (click.x < this.x + this.hwidth && click.x > this.x - this.hwidth &&
            click.y < this.y + this.hwidth && click.y > this.y - this.hwidth) {
            if (origin) {
                //if player two move is valid create link
                if (checkValidMove(this)) {
                    createLink(this);
                } else {
                    //reset origin
...
function checkValidMove(target) {
        ...
        ((origin.gy === target.gy - 2 || origin.gy === target.gy + 2) && origin.gx === target.gx)) //y-move
    {
        ...
        if (origin.gx === target.gx) { //if making a virtical line ensure not crossing
            if (linksArray.find(link =>
        ...
        if (origin.gy === target.gy) { //if making a horizontal line ensuer not crossing
            if (linksArray.find(link =>
        ...
        //do not allow top row or bottom row to make horizontal moves (these are pointless moves)
        if ((origin.gx === 0 && origin.gx === target.gx) || (origin.gx === 10 && origin.gx === target.gx)) {
            return false;
        }

        //do not allow left most or right most column to make virtical moves (these are pointless moves)
